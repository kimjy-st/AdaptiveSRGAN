...
def center_crop_torch(
        gt_images: np.ndarray or torch.tensor or list[np.ndarray] or list[torch.tensor],
        lr_images: np.ndarray or torch.tensor or list[np.ndarray] or list[torch.tensor],
        gt_patch_size: int,
        upscale_factor: int,
) -> [np.ndarray, np.ndarray] or [torch.tensor, torch.tensor] or [list[np.ndarray], list[np.ndarray]] or [list[Tensor], list[Tensor]]:
    """Intercept two images to specify the center area

    Args:
        gt_images (np.ndarray | torch.tensor | list[np.ndarray] | list[torch.tensor]): ground truth images read by PyTorch
        lr_images (np.ndarray | torch.tensor | list[np.ndarray] | list[torch.tensor]): Low resolution images read by PyTorch
    ...
    Returns:
        gt_images (np.ndarray or torch.tensor or): the intercepted ground truth image
        lr_images (np.ndarray or torch.tensor or): low-resolution intercepted images
...
def random_crop_torch(
        gt_images: np.ndarray or Tensor or list[np.ndarray] or list[Tensor],
        lr_images: np.ndarray or Tensor or list[np.ndarray] or list[Tensor],
        gt_patch_size: int,
        upscale_factor: int,
