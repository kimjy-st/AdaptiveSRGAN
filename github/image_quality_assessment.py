...
def _lpips_torch(
    ...
    net : str = "vgg",
    version : str = "0.1",
    device : torch.cuda = "cuda:3"
):
    criterion = LPIPS(net_type="vgg",version=version).to(device)
    loss = criterion(dst_tensor, raw_tensor)
    #loss = lpips(dst_tensor, raw_tensor, net_type="vgg", version = version)
    return loss.item()
...
class LPIPS_Score(nn.Module):
    def __init__(self, net_type, version) -> None:
        super(LPIPS_Score, self).__init__()
        self.net_type = net_type
        self.version = version
        
    def forward(self, raw_tensor:Tensor, dst_tensor:Tensor):
        lpips_metrics = _lpips_torch(raw_tensor, dst_tensor, self.net_type, self.version)
