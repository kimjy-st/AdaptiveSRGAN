...
class SRResNet(nn.Module):
    def __init__(
        ...
        self.upsampling2 = nn.Sequential(*upsampling)
        # reconstruction block
        self.conv3 = nn.Conv2d(channels, out_channels, (9, 9), (1, 1), (4, 4))
        
        #weight map
        self.weight= torch.randn(9)
        self.weight= self.weight.to(device="cuda:3")
        self.outputs = []
        # Initialize neural network weights
        for module in self.modules():
            if isinstance(module, nn.Conv2d):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        #x1은 mse loss, x2는 adv loss
        #final output은 mse + adv loss
        x1 = self.upsampling(x)
        x2 = self.upsampling2(x)
        x1 = self.conv3(x1)
        x2 = self.conv3(x2)
        #make image tensor to patch tensor
        #patch size = (H/3,W/3)
        if len(x1.shape) == 4:
            buffer = []
            for img_index in range(x1.shape[0]):
                x1_patches = self._crop_tensor(x1[img_index])
                x2_patches = self._crop_tensor(x2[img_index])
                sr_patches = self._weight_mul(x1_patches, x2_patches)
                sr = self._ordering(sr_patches)
                buffer.append(sr)
            x = torch.stack(buffer)
        x = torch.clamp_(x, 0.0, 1.0)
        return x, x1, x2
    
    def _crop_tensor(self, input_tensor : Tensor,patch_division = (3,3)):
        outputs = list()
    ...
    def _weight_mul(self,output1, output2):
        srs = []
        for index in range(self.weight.shape[0]):
            patch1 = output1[index]
            patch2 = output2[index]
            sr_patch = self.weight[index]*patch1 + (1-self.weight[index])*patch2
            srs.append(sr_patch)
        srs = torch.stack(srs)
        return srs
    
    def _ordering(self, srs):
        left = torch.cat((srs[0],srs[1],srs[2]), axis = 1)
        median = torch.cat((srs[3],srs[4],srs[5]), axis = 1)
        right = torch.cat((srs[6], srs[7], srs[8]), axis=1)
        
        final_sr = torch.cat((left, median, right), axis=2)
        return final_sr
