...
from add_freqw import buffer_0508 as adv_loss_weight
    ...
    while batch_data is not None:
        ...
        with amp.autocast():
            ...
            for i in range(16):
                ...
                gtpatch, srpatch, freqlist = image_crop(sr_pil, gt_pil)
                srpatchtensor = torch.stack(srpatch)
                gtpatchtensor = torch.stack(gtpatch)
                
                adv_w_list = []
                content_w_list = [] 

                for index in range(len(freqlist)):
                    adv_w = adv_loss_weight(freqlist[index])
                    adv_w_list.append(adv_w)
                    adv_w = 0

                    
                    img_real_label = real_label[i].repeat(1, 1) 
                    srpatchtensor_i = srpatchtensor[index].to(device)
                    srpatchtensor_i = srpatchtensor_i.reshape(1,3,32,32) 
                    
                    gtpatchtensor_i = gtpatchtensor[index].to(device)
                    gtpatchtensor_i = gtpatchtensor_i.reshape(1,3,32,32)
    
                    patch_adv_loss = adversarial_criterion(d_model(srpatchtensor_i),img_real_label).to("cpu")
                    adv_losses.append(patch_adv_loss)
                    
                
                adv_w_list = torch.tensor(adv_w_list)
                content_w_list = torch.tensor(content_w_list)
                adv_losses = torch.stack(adv_losses)
                img_adversarial_loss = adv_w_list * adv_losses

                img_adversarial_loss = img_adversarial_loss.mean()
            ...
            #batch_content_loss = torch.stack(batch_content_loss)
            
            adversarial_loss = torch.mean(batch_adv_loss.to(device)).requires_grad_(True)
